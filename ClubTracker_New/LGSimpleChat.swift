...
class LGChatController : UIViewController, UITableViewDelegate, UITableViewDataSource, LGChatInputDelegate {
    ...
    */
    ...
    private func tableViewConstraints() -> [NSLayoutConstraint] {
        ...
        let bottomConstraint = NSLayoutConstraint(item: tableView, attribute: .bottom, relatedBy: .equal
            , toItem: chatInput, attribute: .top, multiplier: 1.0, constant: 0)
    ...
    private func scrollToBottom() {
        if messages.count > 0 {
            ...
            let lastIndexPath = NSIndexPath.init(row: lastItemIdx, section: Constants.MessagesSection)
//            let lastIndexPath = NSIndexPath(forRow: lastItemIdx, inSection: Constants.MessagesSection)
    ...
    func chatInput(chatInput: LGChatInput, didSendMessage message: String) {
        let newMessage = LGChatMessage.init(content: message, sentByString: LGChatMessage.SentBy.User.rawValue)
//        let newMessage = LGChatMessage(content: message, sentByString: .User)
...
class LGChatInput : UIView, LGStretchyTextViewDelegate {
    ...
    */
    ...
    private let textView = LGStretchyTextView(frame: CGRect.zero, textContainer: nil)
...
class LGStretchyTextView : UITextView, UITextViewDelegate {
    ...
    func align() {
        guard let end = self.selectedTextRange?.end, let caretRect:CGRect = self.caretRect(for: end) else {
            return
        }
