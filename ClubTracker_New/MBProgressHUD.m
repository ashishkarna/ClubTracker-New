...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated full:(BOOL)full{
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.removeFromSuperViewOnHide = YES;
    hud.coverFullView = full;
    [view addSubview:hud];
    [hud show:animated];
...
- (void)layoutSubviews {
	...
	if (parent) {
        ...
        if (!coverFullView) {
            CGFloat topSpace = 65.0;
            frame.origin.y = [UIApplication sharedApplication].isStatusBarHidden ? (topSpace - 20.0f) : topSpace;
